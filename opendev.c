...
void NoBlock(Device *dev)
{
	int	flags;

        fcntl(dev->in, F_GETFL, &flags);
	flags |= FNDELAY;
        fcntl(dev->in, F_SETFL, flags);
}

#if OLD_TERMIO

void SetSpeed(Device *dev, int speed)
{
	int	i, baud = -1;
	struct sgttyb	ttyb;
...
#if 0
	ttyb.sg_flags |= (RAW | ANYP | NOFLSH | NOHANG);
	ttyb.sg_flags |= O_NOHANG;
	ttyb.sg_flags |= O_NOFLSH;
#endif
	ttyb.sg_flags |= (RAW | ANYP );
	if(strcmp(dev->name, "-") != 0) {
		for (i = 0; i < sizeof(bdnum)/sizeof(bdnum[0]); i++) {
			if(speed == bdnum[i]) {
				baud = bdconst[i];
				break;
			}
    	...
    	ioctl(dev->in, TIOCSETN, &ttyb);
#if 0
	if(strcmp(dev->name, "-") != 0) {
    		ioctl(dev->out, TIOCSETN, &ttyb);
	}
#endif
}
...
void SetSpeed(Device *dev, int speed)
{
	...
	ti = dev->origState;
	ti.c_iflag &= ~(ISTRIP | INLCR | ICRNL | IUCLC | IMAXBEL);
	ti.c_iflag |= (IXON | IXOFF | IXANY );
	ti.c_iflag |= (IGNBRK | IGNPAR | PARMRK );
	...
	if(strcmp(dev->name, "-") != 0) {
		...
		ti.c_cflag &= ~CSIZE;
		ti.c_cflag |= CS8;
		ti.c_cflag &= ~(HUPCL | CSTOPB | PARENB);
		ti.c_cflag |= (CREAD | CLOCAL);
		fprintf(log, "Setting terminal cflags to 0x%x\n", ti.c_cflag);
	}

    	ioctl(dev->in, TCSETSW, &ti);
#if 0
    	ioctl(dev->out, TCSETSW, &ti);
#endif
}
...
static void resetDevice(Device *dev)
{
#if OLD_TERMIO
	ioctl(dev->in, TIOCSETN, &dev->origState);
	/* ioctl(dev->out, TIOCSETN, &dev->origState); */
#else
	ioctl(dev->in, TCSETSW, &dev->origState);
	/* ioctl(dev->out, TCSETSW, &dev->origState); */
#endif
}


int openDevice(const char *name, Device *dev)
{
	int	ch;

	dev->name = strdup((char *)name);
	if(strcmp(dev->name, "-") == 0) {
		dev->in = fileno(stdin);
		dev->out = fileno(stdout);
	} else {
		dev->in = open(name, O_RDONLY);
		if(dev->in < 0) {
			fprintf(stderr, "Cannot open device '%s' for input\n",
					name);
			return -1;
		}
		dev->out = open(name, O_WRONLY);
		if(dev->out < 0) {
			fprintf(stderr, "Cannot open device '%s' for output\n",
					name);
			return -1;
		}
	}
	fprintf(log, "Open %s for input on %d, output on %d\n",
			dev->name, dev->in, dev->out);
#if OLD_TERMIO
	ioctl(dev, TIOCGETP, &dev->origState);
#else
	ioctl(dev, TCGETS, &dev->origState);
#endif

	return 0;
}

int closeDevice(Device *dev)
{
	resetDevice(dev);
	if(strcmp(dev->name, "-") == 0) return;
#if 0
	close(dev->in);
	close(dev->out);
#endif
